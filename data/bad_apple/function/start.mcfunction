...
execute if score temp bad_apple_start matches 3 run say spawn axolotle
...
execute if score temp bad_apple_start matches 9 run execute as @a run scoreboard players set @s bad_apple_frame 0
execute if score temp bad_apple_start matches 9 run summon axolotl 0 90 0 {Tags:["bad_apple_marker"],NoAI:1b,Invulnerable:1b,active_effects:[{id:"glowing",amplifier:1,duration:999999,show_particles:0b}]}
execute if score temp bad_apple_start matches 10 run execute at @e[tag=bad_apple_marker] run function bad_apple:spawn_chicken_grid
execute if score temp bad_apple_start matches 10 run say Bad Apple!!
